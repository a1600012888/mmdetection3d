...
from plugin.pointpainting.paint_pts import paint_pts
import numpy as np
import torch
...
from .visulize import visulize, show_mask, get_mask_label

@HEADS.register_module()
class HybridTaskCascadeRoIHeadPainting(CascadeRoIHead):
    ...
    def __init__(self,
                 ...
                 mask_info_flow=True,
                 **kwargs):
        super(HybridTaskCascadeRoIHeadPainting,
    ...
    def forward_train(self,
        ...
        if self.with_semantic:
            ...
            up_semantic_pred = F.interpolate(semantic_pred, (928, 1600), 
                                            mode='bilinear')
            #painted_pts = paint_pts(up_semantic_pred, pts, img_metas)
            #save_pts(painted_pts, img_metas)
    ...
    def simple_test(self, x, proposal_list, img_metas, points, sweep_points, pts_metas, rescale=False):
        ...
        mask_labels = []
        for i in range(num_imgs):
            mask_label = get_mask_label(img_metas[i], segms=segm_results[i])
            mask_labels.append(mask_label)
        
        paint_pts(mask_labels, points, sweep_points, img_metas, pts_metas)
        #for i in range(num_imgs):
        #    show_mask(img_metas[i]['filename'], segms=segm_results[i])
