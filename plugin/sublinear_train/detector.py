...
class Detr3DCamSubLinear(MVXTwoStageDetector):
    ...
    def forward(self, return_loss=True, **kwargs):
        if return_loss:
            ...
            if 'visualize' in kwargs and kwargs['visualize']:
                return self.forward_visualize(**kwargs)
            else:
    ...
    def iterative_test_pts(self, x, img_metas, rescale=False):
        """Test function of point cloud branch."""
        outs = self.pts_bbox_head(x, img_metas)
        bbox_list_all = self.pts_bbox_head.get_iterative_bboxes(
            outs, img_metas, rescale=rescale)

        bbox_results_all = []
        for bbox_list in bbox_list_all:
            bbox_results = [
                bbox3d2result(bboxes, scores, labels)
                for bboxes, scores, labels in bbox_list
            ]
            bbox_results_all.append(bbox_results)
        return bbox_results_all

    def forward_visualize(self, points, img_metas, img=None, rescale=False, **kwargs):
        ...
        bbox_pts_all = self.iterative_test_pts(
            img_feats, img_metas, rescale=rescale)

        for bbox_pts in bbox_pts_all:
