...
def get_motion_metrics(pred, gt, scale=False):
    ...
    thres_3 = torch.sum(epe_map < 0.03) / num
    thres_5 = torch.sum(epe_map < 0.05) / num
    thres_10 = torch.sum(epe_map < 0.1) / num
    #print(epe_map.shape, 'after')
    epe = torch.mean(epe_map)

    epe_rel = torch.sum(epe_map / gt_speed) / num

    return [epe, epe_rel, torch.tensor(ratio).cuda(), thres_3, thres_5, thres_10]
