...
class SpatialTempNet(nn.Module):
    ...
    def __init__(self, depth_net_cfg, sf_net_cfg=None,
        ...
        self.sf_net = SFNet()
        self.photometric_loss = PhotoMetricLoss(w_l1=w_l1, w_census=w_census, reduce='min')
    ...
    def single_forward_train(self, data, **kwargs):
        ...
        rec_prev_imgs = warp_ref_image_temporal(now_disp, prev_imgs,
                                        now_cam_intrin, prev_cam_intrin,
                                        now_cam_pose, prev_cam_pose,
                                        now2prev_sf_pred)

        rec_next_imgs = warp_ref_image_temporal(now_disp, next_imgs,
                                        now_cam_intrin, next_cam_intrin,
                                        now_cam_pose, next_cam_pose,
                                        now2next_sf_pred)

        # [N, 1, H, W]
        temp_rec_loss = self.photometric_loss(now_imgs, [rec_next_imgs, rec_prev_imgs])
