...
def postional_encoding(x_cord, n_L=32):
    '''
    x_cord: [... 3]
    '''
    x = x_cord * 2. - 1
    freq_bands = 2.**torch.linspace(0., n_L//2 - 1, steps=n_L//2, device=x.device)
    
    freq_bands = freq_bands.view([1]*(x.ndim - 1) + [-1])

    x_enc = torch.cat([
        torch.sin(x[..., [0]] * freq_bands), 
        torch.cos(x[..., [0]] * freq_bands)
    ], dim=-1)

    y_enc = torch.cat([
        torch.sin(x[..., [1]] * freq_bands), 
        torch.cos(x[..., [1]] * freq_bands)
    ], dim=-1)
    
    z_enc = torch.cat([
        torch.sin(x[..., [2]] * freq_bands), 
        torch.cos(x[..., [2]] * freq_bands)
    ], dim=-1)


    ret_enc = torch.cat([x_enc, y_enc, z_enc], dim=-1)
...
class Detr3DCamCrossAttenPos(BaseModule):
    ...
    def __init__(self,
                 ...
                 norm_cfg=None,
                 init_cfg=None):
        super(Detr3DCamCrossAttenPos, self).__init__(init_cfg)
        ...
        self.output_proj = nn.Linear(embed_dims+3*dim_enc, embed_dims)
        self.level_embeds = nn.Parameter(
            torch.Tensor(self.embed_dims+3*dim_enc, self.num_levels))
        self.cam_embeds =  nn.Parameter(
            torch.Tensor(self.embed_dims+3*dim_enc, self.num_cams))
    ...
    def forward(self,
        ...
        output, mask = feature_sampling(value, reference_points, offsets, self.pc_range, kwargs['img_metas'], self.dim_enc)
        output = torch.nan_to_num(output)
        mask = torch.nan_to_num(mask)

        level_embeds = self.level_embeds.view(1, self.embed_dims+3*self.dim_enc, 1, 1, 1, self.num_levels)
        cam_embeds = self.cam_embeds.view(1, self.embed_dims+3*self.dim_enc, 1, self.num_cams, 1, 1)
...
def feature_sampling(mlvl_feats, reference_points, offsets, pc_range, img_metas, dim_enc):
    lidar2img = []
    img_flip = []
    ...
    # [B, num_query, 3*dim_enc]
    raw_reference_points = postional_encoding(raw_reference_points, dim_enc)
    
    # to [B, 33*dim_enc, num_query] => [B3*dim_enc, 3, num_query, 1,1]
    raw_reference_points = raw_reference_points.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1)
    # change to [B, 3*dim_enc, num_query, N, num_points]
