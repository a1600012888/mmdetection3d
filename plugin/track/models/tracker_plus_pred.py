...
from plugin.track.models.attention import inverse_sigmoid
import torch
...
class Detr3DCamTrackerPlusLidarVeloPred(MVXTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 num_classes=7,
                 num_frame=3, 
                 num_pred=4, # how many future frames to predict
                 bbox_coder=dict(
        ...
        super(Detr3DCamTrackerPlusLidarVeloPred,
        ...
        self.prediction_layer = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(embed_dims * 2, num_pred * 3)
        )
    ...
    def get_predictions(self, track_instances, time_delta_list):
        matched_instances = track_instances[track_instances.obj_idxes >=0]
        box_residual = self.prediction_layer(matched_instances.output_embedding)

        # List[ tensor of shape: (num_matched_query, 3) ]
        box_residual_list = torch.split(box_residual, 3, dim=-1)

        pred_boxes = matched_instances.pred_boxes.detach()
        # vx, vy
        velocity = pred_boxes[:, -2:]
        velo_pad_ = velocity.new_zeros((pred_boxes.shape[0], 1))
        velo_pad = torch.cat((velocity, velo_pad_), dim=-1)
        ...
        box_pos = torch.cat([cx, cy, cz], dim=-1).detach()

        predictions = []

        for i, time_delta in enumerate(time_delta_list):
            time_delta = time_delta.type(torch.float)
            pred_pos = box_pos + velo_pad * time_delta
            pred_pos = box_residual_list[i] + pred_pos

            predictions.append(pred_pos)

        return matched_instances, predictions
    ...
    def forward_train(self,
        ...
        # for prediction frames, prepare the gt
        l2g_r_mat_pred = l2g_r_mat[num_frame - 1]
        g2l_r_mat_pred = torch.linalg.inv(l2g_r_mat_pred).type(torch.float)
        l2g_t_pred = l2g_t[num_frame - 1]
        for i in range(num_frame, self.num_pred + num_frame):
            gt_instances = Instances((1, 1))
            boxes = gt_bboxes_3d[0][i].tensor.to(img.device)
            boxes_pos = boxes[..., :3]

            # convert box xyz to the last track frame coords
            l2g_r_i = l2g_r_mat[i]
            l2g_t_i = l2g_t[i]

            boxes_pos = boxes_pos @ l2g_r_i + l2g_t_i - l2g_t_pred
            boxes_pos = boxes_pos @ g2l_r_mat_pred
        ...
        time_stamp_pred = timestamp[num_frame - 1]
        for i in range(num_frame, self.num_pred + num_frame):
            time_delta_list.append(timestamp[i] - time_stamp_pred)

        matched_instances, predictions = self.get_predictions(track_instances, time_delta_list)
        self.criterion.prediction_loss(matched_instances, predictions)
