...
class Detr3DCamPtsRadarCrossAtten(BaseModule):
    ...
    def __init__(self,
                 ...
                 batch_first=False,
                 **kwargs):
        super(Detr3DCamPtsRadarCrossAtten, self).__init__(init_cfg)
        ...
        self.img_pts_radar_fusion = nn.Sequential(
            nn.Linear(2*embed_dims+radar_dims, embed_dims),
    ...
    def init_weight(self):
        ...
        xavier_init(self.img_pts_radar_fusion, distribution='uniform', bias=0.)
    ...
    def forward(self,
        ...
        output = torch.cat((output, pts_output, radar_out), dim=-1)
        output = self.img_pts_radar_fusion(output)
...
@ATTENTION.register_module()
class Detr3DRadarCrossAtten(BaseModule):
    """An attention module used in Detr3d. 
    Args:
        embed_dims (int): The embedding dimension of Attention.
            Default: 256.
    ...
    def __init__(self,
                 ...
                 batch_first=False,
                 **kwargs):
        super(Detr3DRadarCrossAtten, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
        ...
        self.radar_attention_weights = nn.Linear(embed_dims, num_levels*num_points)

        self.radar_output_proj = nn.Linear(embed_dims, embed_dims)
    ...
    def forward(self,
        ...
        radar_output= feature_sampling_3D(
            value, reference_points, self.pc_range)
        radar_output = torch.nan_to_num(radar_output)
        
        radar_attention_weights = radar_attention_weights.sigmoid()
        radar_output = radar_output * radar_attention_weights
        radar_output = radar_output.sum(-1).sum(-1).sum(-1)
        radar_output = radar_output.permute(2, 0, 1)
        ...
        return self.dropout(radar_output) + inp_residual + pos_feat
...
class Detr3DCamRadarCrossAtten(BaseModule):
    ...
    def __init__(self,
        ...
        self.radar_attention_weights = nn.Linear(embed_dims, num_radar_levels*num_points)
        self.output_proj = nn.Linear(embed_dims, embed_dims)
        self.radar_output_proj = nn.Linear(radar_dims, radar_dims)
    ...
    def forward(self,
        ...
        radar_attention_weights =  self.radar_attention_weights(query).view(
            bs, 1, num_query, 1, self.num_points, self.num_radar_levels)

        radar_output= feature_sampling_3D(
            radar_feats, reference_points, self.pc_range)
        radar_output = torch.nan_to_num(radar_output)
        ...
        output = torch.cat((output, radar_output), dim=-1)
        output = self.img_radar_fusion(output)
        
        # (num_query, bs, embed_dims)
        pos_feat = self.pos_encoder(
            inverse_sigmoid(reference_points_3d)).permute(1, 0, 2)
...
@ATTENTION.register_module()
class Detr3DCamPtsRadarCrossAttenv2(BaseModule):
    """An attention module used in Detr3d. 
    Args:
        embed_dims (int): The embedding dimension of Attention.
            Default: 256.
    ...
    def __init__(self,
                 ...
                 batch_first=False,
                 **kwargs):
        super(Detr3DCamPtsRadarCrossAttenv2, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
    ...
    def forward(self,
        ...
        output = torch.cat((output, pts_output, radar_output), dim=-1)
        output = self.img_pts_radar_fusion(output)
