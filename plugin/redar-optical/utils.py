...
from .arflow_utils import mesh_grid
...
def get_bidirection_motion_meterics_v2(sf_next_pred, sf_prev_pred, 
                            sf_next, sf_prev,
                            now2next_time, now2prev_time, 
                            now_cam_intrin, now_cam_pose,
                            next_cam_intrin, next_cam_pose,
    ...
    sf_next_mask = sf_next[:, [3], ...]
    scaled_sf_next = torch.cat([scale_sf_next_field, sf_next_mask], dim=1)
    real_sf_next = convert_res_sf_to_opflow(now_cam_intrin, next_cam_intrin,
                                now_cam_pose, next_cam_pose, now_depth_gt, scaled_sf_next)

    scale_sf_prev_field = sf_prev[:, :3, ...] * now2prev_time
    sf_prev_mask = sf_prev[:, [3], ...]
    scaled_sf_prev = torch.cat([scale_sf_prev_field, sf_prev_mask], dim=1)
    real_sf_prev = convert_res_sf_to_opflow(now_cam_intrin, prev_cam_intrin,
                                now_cam_pose, prev_cam_pose, now_depth_gt, scaled_sf_prev)
...
def convert_res_sf_to_opflow(K, ref_K, extrinsics, ref_extrinsics, 
                depth_map, res_sf):
    """
    depth_map: [N, 1, H, W]
    res_sf: [N, 4, H, W]:  valid_mask = res_sf[:,-1,:,:]
    ...
    norm_ref_cords = ref_cams.project(cam_ref_points, 'c')
    ref_cords = (norm_ref_cords + 1.) / 2.0  # [B, H, W, 2] 
    ori_cords = mesh_grid(B, H, W)  # [B, 2, H, W]
    op_flow = ref_cords.permute(0, 3, 1, 2) - ori_cords # [B, 2, H, W]
    op_flow = op_flow * valid_mask

    ret = torch.cat([op_flow, real_scene_flow[:, [2]], valid_mask], dim=1)
