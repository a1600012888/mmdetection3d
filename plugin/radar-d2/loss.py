...
class Charbonnier(nn.Module):
    def __init__(self, p=0.4, eps=1e-2):
        super().__init__()
        self.p = p 
        self.eps = eps
    ...
    def forward(self, x):
        ...
        return torch.pow(x.abs() + self.eps, self.p)
...
class BidirectionalSFRecLoss(nn.Module):
    ...
    def __init__(self, w_l1=0.5, w_census=0.5,  decay=0.8):
        ...
        self.decay = decay
        self.charbonnier_func = Charbonnier(p=0.4, eps=1e-2)
    ...
    def forward(self, sf, rgbd1, rgbd2, cam_intrin1, cam_intrin2):
        ...
        z_loss = torch.mean(self.charbonnier_func(delta_z1 - sf12[:,[2]]) * mask1) + \
                    torch.mean(self.charbonnier_func(delta_z2 - sf21[:,[2]]) * mask2)
