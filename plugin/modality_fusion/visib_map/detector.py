...
from ..save_rangeview import save_bev, save_vismap
...
class VisibilityFeatureExtractor(nn.Module):
    ...
    def forward(self, logodds):
        #save_vismap(logodds.reshape((-1, self.z_size, self.y_size, self.x_size)))
        occupancy = torch.sigmoid(logodds)
        occupancy = occupancy.reshape((-1, self.z_size, self.y_size, self.x_size))
        #save_vismap(occupancy)
        return occupancy

@DETECTORS.register_module()
class Detr3DCamModalityFusionVisMap(MVXTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 visibility_feature_extractor=None,
                 img_backbone=None,
                 pts_backbone=None,
                 img_neck=None,
                 pts_neck=None,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(Detr3DCamModalityFusionVisMap,
        ...
        self.visibility_feature_extractor = VisibilityFeatureExtractor(
            **visibility_feature_extractor
        )

    def extract_pts_feat(self, pts, logodds):
        """Extract features of points."""
        if not self.with_pts_bbox:
            return None

        if isinstance(logodds, list):
            logodds = torch.cat([logodd for logodd in logodds])
        ...
        fused_feat = torch.cat((x, visibility_features), dim=1)
        x = self.pts_backbone(fused_feat)
    ...
    def extract_feat(self, points, logodds, img, img_metas):
        ...
        pts_feats = self.extract_pts_feat(points, logodds)
    ...
    def forward_train(self,
        ...
        """
        # img [N, 3, 928, 1600]
        img_feats, pts_feats = self.extract_feat(points, logodds=logodds,img=img, img_metas=img_metas)
        losses = dict()
    ...
    def simple_test(self, points, img_metas, img=None, logodds=None, rescale=False):
        """Test function without augmentaiton."""
        img_feats, pts_feats = self.extract_feat(
            points, logodds=logodds, img=img, img_metas=img_metas)
...
@DETECTORS.register_module()
class Detr3DCamPointVisMap(MVXTwoStageDetector):
    """Base class of Multi-modality VoxelNet."""
    ...
    def extract_pts_feat(self, pts, logodds):
       ...
       #print('pts', type(pts), len(pts))
        ...
        #print(logodds.shape)
        visibility_features = self.visibility_feature_extractor(logodds)
        #save_vismap(visibility_features)
        #print('visib map', visibility_features.size())
        fused_feat = torch.cat((x, visibility_features), dim=1)
        x = self.pts_backbone(fused_feat)
        if self.with_pts_neck:
            x = self.pts_neck(x)
