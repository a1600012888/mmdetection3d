...
class LoadPointsFromFilev2(object):
    ...
    def __call__(self, results):
        ...
        #print('pts', pts_filename)
        #print('img', results['img_filename'])
        #print('sweeps', len(results['sweeps']))
...
class LoadPointsFromMultiSweepsv2(object):
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: The result dict containing the multi-sweep points data. \
                ...
                - points (np.ndarray | :obj:`BasePoints`): Multi-sweep point \
                    cloud arrays.
        """
        # results['sweeps']: list[dict], including 10 sweep points. Each of sweep dict consist
        # of {data_path, type, sample_data_token, sensor2ego_translation, sensor2ego_rotation,
        # ego2global_translation, timestamp, sensor2lidar_rotation, sensor2lidar_translation}
        # former 9 sweeps are in sweeps root, the last is in samples
        ...
        if self.pad_empty_sweeps and len(results['sweeps']) == 0:
            ...
            for idx in range(len(results['sweeps'])):
        ...
        results['sweep_points'] = sweep_points_list
        pts_metas = []
        for i in range(len(results['sweeps'])):
            pts_meta = {}
            pts_meta['data_path'] = results['sweeps'][i]['data_path']
            pts_meta['sensor2lidar_rotation'] = results['sweeps'][i]['sensor2lidar_rotation']
            pts_meta['sensor2lidar_translation'] = results['sweeps'][i]['sensor2lidar_translation']
            pts_metas.append(pts_meta)

        results['pts_metas'] = pts_metas
        return results
...
@PIPELINES.register_module()
class LoadPaintedPointsFromMultiSweeps(object):
    """Load points from multiple sweeps.
    This is usually used for nuScenes dataset to utilize previous sweeps.
    Args:
        sweeps_num (int): Number of sweeps. Defaults to 10.
...
@PIPELINES.register_module()
class LoadPaintedPointsFromFile(object):
    """Load Points From File.
    Load sunrgbd and scannet points from file.
    Args:
        coord_type (str): The type of coordinates of points cloud.
...
class DataBaseSamplerPainted(object):
