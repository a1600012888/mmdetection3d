...
class SpatialTempNet(nn.Module):
    ...
    def forward_train(self, data, **kwargs):
        ...
        loss_list = []
        log_vars_list = []
        now2next_sf_pred = None
        now2prev_sf_pred = None
        for pred_idx, disp_raw in enumerate(all_disp_preds):
            #from IPython import embed
            #embed()
            ...
            if now2next_sf_pred is None:
                prev_imgs_depth = torch.cat([prev_imgs, prev_depth_pred], dim=1)
                next_imgs_depth = torch.cat([next_imgs, next_depth_pred], dim=1)
                
                rec_prev_imgs_no_sf = warp_ref_image_temporal(now_inv_depth / depth_scale, prev_imgs_depth,
                                                now_cam_intrin, prev_cam_intrin,
                                                now_cam_pose, prev_cam_pose,
                                                #now2prev_sf_pred)
                                                None)
                                                #
                rec_next_imgs_no_sf = warp_ref_image_temporal(now_inv_depth / depth_scale, next_imgs_depth,
                ...
                now2next_inp = torch.cat([now_imgs_depth.detach(), rec_next_imgs_no_sf.detach(), next_imgs_depth], dim=1)
                now2prev_inp = torch.cat([prev_imgs_depth.detach(), rec_prev_imgs_no_sf.detach(), prev_imgs_depth], dim=1)
                now2next_sf_pred = self.sf_net(now2next_inp)
                now2prev_sf_pred = self.sf_net(now2prev_inp)

            rec_prev_imgs = warp_ref_image_temporal(now_inv_depth.detach() / depth_scale, prev_imgs,
                                            now_cam_intrin, prev_cam_intrin,
                                            now_cam_pose, prev_cam_pose,
                                            now2prev_sf_pred)
                                            #None)
                                            #
            rec_next_imgs = warp_ref_image_temporal(now_inv_depth.detach() / depth_scale, next_imgs,
                                            now_cam_intrin, next_cam_intrin,
                                            now_cam_pose, next_cam_pose,
                                            now2next_sf_pred)
                                            #None)
            ...
            temp_rec_loss_maps = [torch.mean(torch.abs(rec_prev_imgs - now_imgs), dim=1, keepdim=True),
                                torch.mean(torch.abs(rec_next_imgs - now_imgs), dim=1, keepdim=True),
                                torch.mean(torch.abs(prev_imgs - now_imgs), dim=1, keepdim=True),
                                torch.mean(torch.abs(next_imgs - now_imgs), dim=1, keepdim=True), ]
            temp_rec_loss_map = torch.cat(temp_rec_loss_maps, dim=1)
            temp_rec_loss_map, _ = torch.min(temp_rec_loss_map, dim=1)

            temp_rec_loss = torch.mean(temp_rec_loss_map)
        ...
        with torch.no_grad():
            ...
            motion_metrics = get_motion_metrics(fused_motion,  # time_delat: 0.5s / 0.1s for key/non-key
                                                now_sf/10.0, scale=self.scale_depth)
