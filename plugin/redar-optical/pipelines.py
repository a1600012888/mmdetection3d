...
class LoadSceneFlows(object):
    ...
    def naive_sceneflow_render(self, points, sf_map, valid_mask):
        ...
        return sf_map.astype(np.float32)
    ...
    def __call__(self, results):
        ...
        for token in tokens:
            npy_file_paths = results['npy_info']['sf_{}_paths'.format(token)]
            
            i = 0
            for npy_file_path in npy_file_paths:
                if npy_file_path is None:
                    sf_map = np.zeros((*self.img_size, 4))
                    results['seg_fields'].append('sf_{}_map{}'.format(token, i))
                    results['sf_{}_map{}'.format(token, i)] = sf_map
                    i += 1
                    continue
...
class LoadDepthPred(object):
    ...
    def __call__(self, results):
        ...
        depth_pred_path = results['npy_info']['depth_pred_path']
        
        depth_pred = np.load(depth_pred_path) # [18, 1, H, W]

        # prev, now, next = np.split(depth_pred, 6, axis=0)

        results['depth_pred'] = depth_pred
        results['seg_fields'].append('depth_pred')
