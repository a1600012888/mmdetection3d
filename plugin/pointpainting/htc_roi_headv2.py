...
from .visulize import visulize, imshow_det_bboxes, show_mask

@HEADS.register_module()
class HybridTaskCascadeRoIHeadv2(CascadeRoIHead):
    ...
    def __init__(self,
                 ...
                 mask_info_flow=True,
                 **kwargs):
        super(HybridTaskCascadeRoIHeadv2,
    ...
    def forward_train(self,
        ...
        if self.with_semantic:
            ...
            up_semantic_pred = F.interpolate(semantic_pred, (928, 1600), 
                                            mode='bilinear')
            '''
            print('pred', semantic_pred.size())
            print('gt', gt_semantic_seg.size())
            print('shape', img_metas[0]['img_shape'], img_metas[1]['img_shape'])
            B, C, H, W = semantic_pred.size()
            
            #up_semantic_pred = up_semantic_pred.view(B, C, 928, 1600)
            seg_acc = accuracy(up_semantic_pred, gt_semantic_seg.squeeze())
            visulize(up_semantic_pred, gt_semantic_seg, img_metas)
            print(seg_acc)
            losses['seg_acc'] = seg_acc
            '''
            loss_seg = self.semantic_head.loss(up_semantic_pred, gt_semantic_seg)
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        if self.with_semantic:
            ...
            #up_semantic_pred = F.interpolate(semantic_pred, (928, 1600), mode='bilinear')
            #visulize(up_semantic_pred, img_metas)
        else:
            semantic_feat = None
        ...
        # rois shape (num_bbox, 5)  5 represent (img_ind, x1, y1, x2, y2)
        ...
        # det_bboxes list[Tensor]. Length of list is num_imgs, tensor shape (num_bboxes, 5)
        # det_labels list[Tensor]. Length of list is num_imgs, tensor shape (num_bboxes)
        bbox_result = [
                ...
                for i in range(num_imgs):
                    if det_bboxes[i].shape[0] == 0:
                        ...
                        merged_mask = merge_aug_masks(
                            ...
                            rcnn_test_cfg)
                        # segm_result list[list]: encoded masks. The c-th item in the outer list
        ...
        #out_file='saved_imgs/' + 'test_' + img_path.split('/')[0]
        #bboxes_np = det_bboxes[0].cpu().numpy()
        #labels_np = det_labels[0].cpu().numpy()
        #segms_np = [[item.cpu().numpy() for item in segm_cls] for segm_cls in segm_results[0]]
        #show_mask(img_path, labels=labels_np, 
        #        segms=segm_results[0], out_file=out_file)
        #print(img_path)
        #results2 = (ms_bbox_result, ms_segm_result)
        return results
