...
def reduce_LiDAR_beamsv2(pts, reduce_beams_to=32):
    #print(pts.size())
    ...
    if reduce_beams_to == 16:
        ...
        mask = (theta < (beam_range[10]-0.012)) * (theta > (beam_range[14]-0.012))
...
class LoadReducedPaintedPointsFromMultiSweepsv2(object):
    ...
    def __call__(self, results):
        ...
        if self.pad_empty_sweeps and len(results['sweeps']) == 0:
            ...
            for idx in choices:
                ...
                points_sweep = reduce_LiDAR_beamsv2(points_sweep, self.reduce_beams_to)
...
@PIPELINES.register_module()
class LoadReducedPaintedPointsFromFilev2(object):
    ...
    def __call__(self, results):
        ...
        points = points[:, self.use_dim]
        points = reduce_LiDAR_beamsv2(points, self.reduce_beams_to)
        points = points.numpy()
        attribute_dims = None
...
class DataBaseSamplerReducedPaintedv2(object):
        ...
        if sum_sampled > 0:
            ...
            for i in range(len(self.sample_classes)):
                for info in sampled[i]:
                    ...
                    pts = s_points.tensor
                    pts = reduce_LiDAR_beamsv2(pts, self.reduce_beams_to)
                    num_pts, _ = pts.size()
