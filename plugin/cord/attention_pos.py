...
class Detr3DCamCrossAttenPos(BaseModule):
    ...
    def __init__(self,
        ...
        self.pos_embed_subnet = nn.Sequential(
            nn.Linear(3*pos_embed_dims, pos_embed_dims),
            #nn.ReLU(),
            #nn.Linear(pos_embed_dims, pos_embed_dims),
        )
        ...
        self.output_proj = nn.Linear(embed_dims+pos_embed_dims, embed_dims)
    ...
    def init_weight(self):
        ...
        xavier_init(self.pos_embed_subnet, distribution='uniform', bias=0.)
    ...
    def forward(self,
        ...
        if self.use_dconv:
            ...
            # change to [num_quer, bs, embed_dims]
        ...
        # print(pos_encoding.shape, output.shape)
        output = torch.cat([output, pos_encoding], dim=-1)
