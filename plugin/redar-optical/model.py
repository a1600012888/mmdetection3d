...
from .utils import get_depth_metrics, get_motion_metrics, remap_invdepth_color,\
    ...
    flow2rgb, convert_res_sf_to_sf, sf_sparse_loss, get_bidirection_motion_meterics_v2, \
    sceneflow2rgb

from .loss import PhotoMetricLoss, UnFlowLoss
...
class SpatialTempNet(nn.Module):
    ...
    def __init__(self, depth_net_cfg, sf_net_cfg=None,
        ...
        self.sf_net = SFNet()
        self.unsup_flow_loss = UnFlowLoss(w_l1=w_l1, w_census=w_census, reduce='min')
    ...
    def forward_train(self, data, **kwargs):
        ...
        now2next_flow_preds = self.sf_net(now2next_inp)
        now2prev_flow_preds = self.sf_net(now2prev_inp)

        
        temp_rec_loss, motion_smoothing_loss, out_dict = self.unsup_flow_loss(prev_imgs, now_imgs, next_imgs, 
                                                        now2next_flow_preds, now2prev_flow_preds)
        ...
        with torch.no_grad():
            ...
            rec_prev_imgs = out_dict['rec_prev_imgs'] * std + mean
            rec_prev_imgs = rec_prev_imgs / 255.0

            rec_next_imgs = out_dict['rec_next_imgs'] * std + mean
            ...
            now2next_sf_preds = self.sf_net.upgrade_flow(now2next_inp, now2next_flow_preds)
            now2prev_sf_preds = self.sf_net.upgrade_flow(now2prev_inp, now2prev_flow_preds)
            
            motion_metrics = get_bidirection_motion_meterics_v2(now2next_sf_preds, now2prev_flow_pred, 
