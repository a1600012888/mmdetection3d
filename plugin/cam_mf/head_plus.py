...
class DeformableDETR3DCamMulFrameHead(DETRHead):
    ...
    def forward(self, mf_mlvl_feats, img_metas):
        """Forward function.
        Args:
            mf_mlvl_feats (tuple[Tensor]): Features from the upstream
        ...
        batch_size = mf_mlvl_feats[0][0].size(0)
        batch_size = mf_mlvl_feats[0][0].size(0)
        #input_img_h, input_img_w = img_metas[0]['batch_input_shape']
        input_img_h, input_img_w, _ = img_metas[0]['img_shape'][0]
        img_masks = mf_mlvl_feats[0][0].new_ones(
            (batch_size, input_img_h, input_img_w))
        for img_id in range(batch_size):
            img_h, img_w, _ = img_metas[img_id]['img_shape'][0]
            img_masks[img_id, :img_h, :img_w] = 0

        for _frame, mlvl_feats in enumerate(mf_mlvl_feats):
            mlvl_feat_list = []
            for i, feat in enumerate(mlvl_feats):
                ...
                # [B, embed_dim, H, W]
                pos_enc = self.positional_encoding(pos_mask)
                ...
                mlvl_feat_list.append(feat + pos_enc)
            mf_mlvl_feats[_frame] = mlvl_feat_list
