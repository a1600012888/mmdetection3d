...
class Detr3DCamCrossAttenOffsetsFix(BaseModule):
    ...
    def __init__(self,
                 ...
                 norm_cfg=None,
                 init_cfg=None):
        super(Detr3DCamCrossAttenOffsetsFix, self).__init__(init_cfg)
    ...
    def forward(self,
        ...
        # offsets = self.offsets(query).view(bs, 1, num_query, 1, self.num_heads, self.num_points, 3)  # or not?
        offsets = query.new_tensor([ [0.01, 0.01, 0.0], [0.01, -0.01, 0.0], [-0.01, 0.01, 0.0], [-0.01, -0.01, 0.0]])
        offsets = offsets.view(1, 1, 1, 1, 1, self.num_points, 3)
        # shape [B, num_cam, num_query, num_level, num_heads, num_points, 3]
        offsets = offsets.repeat(bs, self.num_cams, num_query, self.num_levels, self.num_heads, 1, 1)
        ...
        if self.use_dconv:
            ...
            #print(mask.type(torch.float).mean().item(), offsets.mean().item(), reference_points.mean().item(), inverse_sigmoid(reference_points).mean().item())
